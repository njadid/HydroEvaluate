...
# DATA LOCATIONS FOR RESULTS (res_ol_dir) AND USGS GAGE DATA (usgs_data_dir)
res_ol_dir = 'E:/APPFiles/PostGreSql/EnterpriseDB-ApacheHTTPD/apache/www/smap/data/results'
res_richards_dir = 'G:/hlm_richards_experiment/Q'
usgs_data_dir = 'E:/APPFiles/PostGreSql/EnterpriseDB-ApacheHTTPD/apache/www/smap/data/usgs_gages_long'

sim_list = ['Q_ol', '_Richards_hlm', '_Richards_hlm_const']
...
def calc_metrics(lid):
    ...
    df_ol = pd.read_csv(path.join(res_ol_dir, 'csv_ol/' + str(lid) + '.csv'), header=0, parse_dates=['dt'],
                        index_col=False)
    ...
    try:
        df_ol1 = pd.read_csv(path.join(res_ol_dir, 'out_ol_new/lid_O_' + str(lid) + '.csv'), header=0,
    ...
    for year in [2016, 2017, 2018]:
        ...
        try:
            df_assim = pd.read_csv(path.join(res_richards_dir, '_Richards_hlm', str(year), str(lid) + '.csv'), header=0, parse_dates=['dt'],
                                index_col=False)
            df_assim_norm = pd.read_csv(path.join(res_richards_dir, '_Richards_hlm_const', str(year), str(lid) + '.csv'), header=0,
                                        parse_dates=['dt'], index_col=False)
            ...
            data['Q'] = df_meas
            data['Q_ol'] = df_ol
            data['_Richards_hlm'] = df_assim
            data['_Richards_hlm_const'] = df_assim_norm
...
df_final1.to_csv('df_final_new.csv', float_format='%.3f',index=False)
